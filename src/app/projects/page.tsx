      ...
      <main className="flex-1 max-w-4xl mx-auto px-8 py-10 space-y-8">
        <header className="space-y-1.5">
          <div className="flex items-center gap-2 text-[#D8B4BE] text-xs font-mono uppercase tracking-widest">
            <FolderKanban className="w-4 h-4 text-[#F6E8EA]" />
            <span>High-Level Contexts</span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#FCF8F9]">Projects</h1>
        ...
        <form onSubmit={handleCreate} className="glass-panel p-5 rounded-2xl space-y-3">
          <div className="flex items-center justify-between text-xs text-[#D8B4BE] font-semibold">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-1.5 bg-[#F6E8EA] hover:bg-[#FCF8F9] text-[#1A0A0F] font-semibold px-4 py-2 rounded-xl text-xs disabled:opacity-30 transition"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {projects.map((proj) => (
            <div key={proj.id} className="glass-card p-5 rounded-2xl space-y-2">
              <div className="flex items-center gap-2.5">
                <Folder className="w-4 h-4 text-[#F6E8EA]" />
                <h3 className="font-bold text-[#FCF8F9]">{proj.name}</h3>
              </div>
              {proj.description && (
                <p className="text-xs text-[#A38F99] leading-relaxed">{proj.description}</p>
              )}
            </div>
          ))}
          ...
          {!loading && projects.length === 0 && (
            <div className="col-span-full text-center py-12 glass-panel rounded-2xl p-6">
              <FolderKanban className="w-7 h-7 text-[#F6E8EA]/60 mx-auto mb-2" />
              <p className="text-sm font-semibold text-[#FCF8F9]">No projects yet</p>
              <p className="text-xs text-[#A38F99] mt-1">Create your first project above to group related tasks.</p>
            </div>
          )}
        </div>
      </main>
